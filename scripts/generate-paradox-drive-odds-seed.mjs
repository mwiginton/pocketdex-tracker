...
const outputPath =
  process.argv[3] ?? "supabase/paradox-drive/seed-paradox-drive-odds.sql";

const setId = "B3a";
const setName = "Paradox Drive";
const packId = "b3a-paradox-drive";
const totalCardCount = 109;
const mainSetCardCount = 74;
const htmlPath = `${inputDir}/b3a-limitless.html`;

const regularRates = {
  diamond_1: [0.03125, 0.03125, 0.03125, 0, 0],
  diamond_2: [0, 0, 0, 0.03599, 0.02399],
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 94 && card.collectorNumber <= 103) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 104 && card.collectorNumber <= 107) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Paradox Drive Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B3a-094 through B3a-103 use shiny_1 rates and cards B3a-104 through B3a-107 use shiny_2 rates; probability means at least once per opened pack.",
...
begin
  ...
  if paradox_drive_cards <> ${totalCardCount} then
    raise exception
  ...
  if paradox_drive_odds <> ${rows.length} then
    raise exception
