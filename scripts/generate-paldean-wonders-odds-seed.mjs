...
const outputPath =
  process.argv[3] ?? "supabase/seed-paldean-wonders-odds.sql";

const setId = "B2a";
const setName = "Paldean Wonders";
const packId = "b2a-paldean-wonders";
const totalCardCount = 131;
const mainSetCardCount = 93;
const htmlPath = `${inputDir}/b2a-limitless.html`;

const regularRates = {
  diamond_1: [0.02325, 0.02325, 0.02325, 0, 0],
  diamond_2: [0, 0, 0, 0.02727, 0.01818],
  diamond_3: [0, 0, 0, 0.00416, 0.01666],
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 116 && card.collectorNumber <= 125) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 126 && card.collectorNumber <= 129) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Paldean Wonders Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B2a-116 through B2a-125 use shiny_1 rates and cards B2a-126 through B2a-129 use shiny_2 rates; probability means at least once per opened pack.",
...
-- https://bulbapedia.bulbagarden.net/wiki/Paldean_Wonders_(TCG_Pocket)#Offering_rates
...
--
-- Published Paldean Wonders pack-type rates:
--   regular pack:            94.711%
...
-- number: B2a-116 through B2a-125 are shiny_1 and B2a-126 through B2a-129
-- are shiny_2.
...
do $$
declare
  paldean_wonders_cards integer;
  paldean_wonders_packs integer;
  paldean_wonders_odds integer;
begin
  select count(*)
  into paldean_wonders_cards
  from public.cards
  where set_id = '${setId}';

  select count(*)
  into paldean_wonders_packs
  from public.packs
  where set_id = '${setId}';

  select count(*)
  into paldean_wonders_odds
  from public.card_pack_odds
  where pack_id = '${packId}';

  if paldean_wonders_cards <> ${totalCardCount} then
    raise exception
      'Expected ${totalCardCount} ${setName} cards, found %',
      paldean_wonders_cards;
  end if;

  if paldean_wonders_packs <> 1 then
    raise exception
      'Expected 1 ${setName} pack, found %',
      paldean_wonders_packs;
  end if;

  if paldean_wonders_odds <> ${rows.length} then
    raise exception
      'Expected ${rows.length} ${setName} odds rows, found %',
      paldean_wonders_odds;
  end if;
end
$$;
