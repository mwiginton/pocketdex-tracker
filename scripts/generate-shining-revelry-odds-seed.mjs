...
const outputPath = process.argv[3] ?? "supabase/seed-shining-revelry-odds.sql";

const setId = "A2b";
const setName = "Shining Revelry";
const packId = "a2b-shining-revelry";
const totalCardCount = 111;
const mainSetCardCount = 72;
const htmlPath = `${inputDir}/a2b-limitless.html`;

const regularRates = {
  diamond_1: [0.03125, 0.03125, 0.03125, 0, 0],
  diamond_2: [0, 0, 0, 0.04045, 0.02545],
  diamond_3: [0, 0, 0, 0.0055, 0.02201],
  diamond_4: [0, 0, 0, 0.00185, 0.0074],
  star_1: [0, 0, 0, 0.00428, 0.01714],
  star_2: [0, 0, 0, 0.00029, 0.00117],
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 97 && card.collectorNumber <= 106) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 107 && card.collectorNumber <= 110) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Shining Revelry Offering Rates and Limitless card list; cards 97-106 use shiny_1 rates and cards 107-110 use shiny_2 rates; probability means at least once per opened pack.",
...
begin
  ...
  if shining_revelry_cards <> ${totalCardCount} then
    raise exception
  ...
  if shining_revelry_odds <> ${rows.length} then
    raise exception
