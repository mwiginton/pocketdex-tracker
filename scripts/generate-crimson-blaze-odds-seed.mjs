...
const outputPath =
  process.argv[3] ?? "supabase/crimson-blaze/seed-crimson-blaze-odds.sql";

const setId = "B1a";
const setName = "Crimson Blaze";
const packId = "b1a-crimson-blaze";
const totalCardCount = 103;
const mainSetCardCount = 69;
const htmlPath = `${inputDir}/b1a-limitless.html`;

const regularRates = {
  diamond_1: [0.03125, 0.03125, 0.03125, 0, 0],
  diamond_2: [0, 0, 0, 0.03749, 0.025],
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 88 && card.collectorNumber <= 97) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 98 && card.collectorNumber <= 101) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Crimson Blaze Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B1a-088 through B1a-097 use shiny_1 rates and cards B1a-098 through B1a-101 use shiny_2 rates; probability means at least once per opened pack.",
...
begin
  ...
  if crimson_blaze_cards <> ${totalCardCount} then
    raise exception
  ...
  if crimson_blaze_odds <> ${rows.length} then
    raise exception
