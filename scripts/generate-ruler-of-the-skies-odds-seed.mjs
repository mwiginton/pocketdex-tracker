...
const outputPath =
  process.argv[3] ??
  "supabase/ruler-of-the-skies/seed-ruler-of-the-skies-odds.sql";

const setId = "B4";
const setName = "Ruler of the Skies";
const packId = "b4-ruler-of-the-skies";
const totalCardCount = 233;
const mainSetCardCount = 155;
const htmlPath = `${inputDir}/b4-limitless.html`;

const regularRates = {
  diamond_1: [0.01515, 0.01515, 0.01515, 0, 0],
  diamond_2: [0, 0, 0, 0.01764, 0.01176],
  diamond_3: [0, 0, 0, 0.00178, 0.00714],
  diamond_4: [0, 0, 0, 0.00166, 0.00666],
  star_1: [0, 0, 0, 0.00107, 0.00428],
  star_2: [0, 0, 0, 0.00022, 0.0009],
  star_3: [0, 0, 0, 0.00111, 0.00444],
  shiny_1: [0, 0, 0, 0, 0],
  shiny_2: [0, 0, 0, 0, 0],
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 204 && card.collectorNumber <= 223) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 224 && card.collectorNumber <= 231) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Ruler of the Skies Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B4-204 through B4-223 use shiny_1 rates and cards B4-224 through B4-231 use shiny_2 rates; probability means at least once per opened pack.",
      )}, date '2026-07-30')${index === rows.length - 1 ? "" : ","}`,
  )
  .join("\n");
...
begin
  ...
  if ruler_of_the_skies_cards <> ${totalCardCount} then
    raise exception
  ...
  if ruler_of_the_skies_odds <> ${rows.length} then
    raise exception
