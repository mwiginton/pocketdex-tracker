...
const outputPath =
  process.argv[3] ??
  "supabase/team-rockets-ambition/seed-team-rockets-ambition-odds.sql";

const setId = "B4a";
const setName = "Team Rocket's Ambition";
const packId = "b4a-team-rockets-ambition";
const totalCardCount = 110;
const mainSetCardCount = 72;
const htmlPath = `${inputDir}/b4a-limitless.html`;
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 95 && card.collectorNumber <= 104) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 105 && card.collectorNumber <= 108) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from the standard themed-booster Offering Rates structure and the Team Rocket's Ambition Limitless/Bulbapedia card list; includes regular, regular-plus-bonus, and rare pack paths; cards B4a-095 through B4a-104 use shiny_1 rates and cards B4a-105 through B4a-108 use shiny_2 rates; probability means at least once per opened pack.",
      )}, date '2026-08-29')${index === rows.length - 1 ? "" : ","}`,
  )
  .join("\n");
...
begin
  ...
  if team_rockets_ambition_cards <> ${totalCardCount} then
    raise exception
  ...
  if team_rockets_ambition_odds <> ${rows.length} then
    raise exception
