...
const outputPath =
  process.argv[3] ?? "supabase/seed-pulsing-aura-odds.sql";

const setId = "B3";
const setName = "Pulsing Aura";
const packId = "b3-pulsing-aura";
const totalCardCount = 234;
const mainSetCardCount = 155;
const htmlPath = `${inputDir}/b3-limitless.html`;

const regularRates = {
  diamond_1: [0.01538, 0.01538, 0.01538, 0, 0],
  diamond_2: [0, 0, 0, 0.01764, 0.01176],
  diamond_3: [0, 0, 0, 0.00172, 0.00689],
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Pulsing Aura Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B3-205 through B3-224 use shiny_1 rates and cards B3-225 through B3-232 use shiny_2 rates; probability means at least once per opened pack.",
...
begin
  ...
  if pulsing_aura_cards <> ${totalCardCount} then
    raise exception
  ...
  if pulsing_aura_odds <> ${rows.length} then
    raise exception
