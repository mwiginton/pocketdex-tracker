...
const outputPath =
  process.argv[3] ?? "supabase/seed-fantastical-parade-odds.sql";

const setId = "B2";
const setName = "Fantastical Parade";
const packId = "b2-fantastical-parade";
const totalCardCount = 234;
const mainSetCardCount = 155;
const htmlPath = `${inputDir}/b2-limitless.html`;
...
function mapOddsRarity(card) {
  if (card.collectorNumber >= 205 && card.collectorNumber <= 224) {
    return "shiny_1";
  }

  if (card.collectorNumber >= 225 && card.collectorNumber <= 232) {
    return "shiny_2";
  }
  ...
  .map(
    (row, index) =>
      `  (${sqlString(row.packId)}, ${sqlString(row.cardId)}, ${row.pullProbability.toFixed(
        ...
        "Derived from Fantastical Parade Offering Rates and Limitless card list; includes regular, regular-plus-bonus, and rare pack paths; cards B2-205 through B2-224 use shiny_1 rates and cards B2-225 through B2-232 use shiny_2 rates; probability means at least once per opened pack.",
...
-- https://bulbapedia.bulbagarden.net/wiki/Fantastical_Parade_(TCG_Pocket)#Offering_rates
...
--
-- Published Fantastical Parade pack-type rates:
--   regular pack:            94.711%
...
-- number: B2-205 through B2-224 are shiny_1 and B2-225 through B2-232 are
-- shiny_2.
...
do $$
declare
  fantastical_parade_cards integer;
  fantastical_parade_packs integer;
  fantastical_parade_odds integer;
begin
  select count(*)
  into fantastical_parade_cards
  from public.cards
  where set_id = '${setId}';

  select count(*)
  into fantastical_parade_packs
  from public.packs
  where set_id = '${setId}';

  select count(*)
  into fantastical_parade_odds
  from public.card_pack_odds
  where pack_id = '${packId}';

  if fantastical_parade_cards <> ${totalCardCount} then
    raise exception
      'Expected ${totalCardCount} ${setName} cards, found %',
      fantastical_parade_cards;
  end if;

  if fantastical_parade_packs <> 1 then
    raise exception
      'Expected 1 ${setName} pack, found %',
      fantastical_parade_packs;
  end if;

  if fantastical_parade_odds <> ${rows.length} then
    raise exception
      'Expected ${rows.length} ${setName} odds rows, found %',
      fantastical_parade_odds;
  end if;
end
$$;
